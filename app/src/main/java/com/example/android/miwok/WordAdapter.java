...
private int mColorResourceId;
    ...
    @Override
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            defaultTextView.setText(word_item.getMiwokTranslation());

            // Find the TextView in the list_item.xml layout with the ID version_number
            TextView miwokTextView = (TextView) listItemView.findViewById(R.id.default_text_view);
            // Get the version number from the current AndroidFlavor object and
            // set this text on the number TextView
            miwokTextView.setText(word_item.getDefaultTranslation());
        ImageView imageView = (ImageView) listItemView.findViewById(R.id.image);

        if (word_item.hasImage()){
            // Set the ImageView to the image resource specified in the current Word
            imageView.setImageResource(word_item.getImageResourceId());
